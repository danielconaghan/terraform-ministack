...
- **ministack** — emulates AWS services (Lambda, API Gateway, IAM) inside Docker
...
| `make up` | Start ministack via Docker Compose and wait until ready |
...
1. **ministack** runs inside Docker and emulates AWS services (Lambda, API Gateway, IAM) on `http://localhost:4566`.
2. **Bref** provides the PHP runtime as a Lambda layer. The `layer` target pulls the official Docker image, extracts `/opt`, and publishes the zip to ministack.
